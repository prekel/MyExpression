...
module BinarySearch =
    let private median (Interval interval) =
        match interval with
    ...
    let search f eps interval =
        ...
        let endsDifference =
            match interval |> Interval.toTuple with
        ...
        let isIntervalValueLessEps =
            match interval |> Interval.toTuple with
            | (left, _) when abs (f left) < eps -> ValueSome left
            | (_, right) when abs (f right) < eps -> ValueSome right
            | _ -> ValueNone
        ...
        let increaseInterval f interval =
            let increase k rl cnd =
                ...
                if (System.Double.IsInfinity(t)) then
                    unreached ()
                else
                    t
        ...
        let rec recSearch interval =
            let localMedian = median interval
            let medianValue = f localMedian

            if Interval.difference interval < eps then
                localMedian
            elif comparer medianValue 0. then
                recSearch
                ^ Interval(interval |> Interval.left, localMedian)
            else
                recSearch
                ^ Interval(localMedian, interval |> Interval.right)
